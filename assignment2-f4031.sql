...
SELECT	CTE.customer_id, CTE.first_name, CTE.last_name, 
		ISNULL (NULLIF (ISNULL(T1.first_name, 'No'), T1.first_name) , 'Yes') as Other_Product
FROM	CTE 
		LEFT JOIN  
		T1
		ON	CTE.customer_id = T1.customer_id;
...
SELECT	CT.adv_type, CAST (ROUND (1.0*CT1.cnt_order_actions / CT.cnt_action, 2) AS numeric (3,2)) AS Conversion_Rate 
FROM	CT, CT1 
WHERE	CT.adv_type = CT1.adv_type;